...
func (w *FQLWorker) Process() ([]byte, error) {
	...
	ctx = drivers.WithContext(ctx, cdp.NewDriver(cdp.WithAddress(w.job.Query.CDPAddress)))
	w.cancel = cancelFn
	...
	program, err := w.compiler.Compile(w.job.Query.Text)

	if err != nil {
		return nil, err
	}

	params := make(map[string]interface{}, len(w.job.Query.Params))

	w.mu.Lock()
	for k, v := range w.job.Query.Params {
		params[k] = v
	}
	w.mu.Unlock()
